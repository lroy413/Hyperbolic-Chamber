...
`wrangler.toml`, `.github/workflows/deploy.yml` and `.gitignore` now exist, and `package-lock.json` is committed so CI installs the versions the suites were run against. The workflow is one job: `npm ci` → build → lint → install Chromium → test → audit → `wrangler deploy`, with the deploy step gated on a push to `main` so pull requests get everything except the upload. It is one job rather than two so the artifact that ships is the exact `dist/` the suites just ran against, and the browser download sits after the two steps that do not need it so a broken build fails in seconds.

`npm run build` launches no browser at all now — the PWA icons moved to `static/icons/` and are committed, regenerated by hand with `npm run icons`. A build runner needs Node and nothing else, which is what makes Cloudflare's own Workers Builds a viable second path: build command `npm ci && npm run build`, deploy command `npx wrangler deploy`. Note that running both that and the GitHub Action means two deploys racing on every push to `main`, and only the Action runs the tests.
