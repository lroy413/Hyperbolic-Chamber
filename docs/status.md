...
### CI and deploy wiring (written, never run end to end)

**Cloudflare Workers Builds deploys.** It watches this repo, with build command `npm ci && npm run build` and deploy command `npx wrangler deploy`, production branch `main`. Its configuration lives in the Cloudflare dashboard and not in this repo, so it cannot be reviewed here — worth knowing when the build behaves differently from what `package.json` suggests.

**The GitHub Action does not deploy.** `.github/workflows/check.yml` is one job — `npm ci` → build → lint → install Chromium → test → audit — on every push and pull request. The browser download sits after the two steps that do not need it, so a broken build fails in seconds rather than after 150 MB. `.gitignore` covers `dist/`, and `package-lock.json` is committed so CI installs the versions the suites were run against.

`npm run build` launches no browser at all — the PWA icons moved to `static/icons/` and are committed, regenerated by hand with `npm run icons`. A build runner needs Node and nothing else, which is what makes Workers Builds workable.

**The tests do not gate the deploy.** Workers Builds runs none of them, so a red check reports a problem rather than preventing it. The fix, if you want the gate, is a branch protection rule on `main` requiring `check` to pass — the barrier moves to the merge. Not set up.

**Not yet proven end to end.** Before the first production build:

- `main` still holds the old flat repo — no `package.json` — so a production build fails at `npm ci` until this branch is merged. Merge first, then watch the build.
...
**1. Merge to `main`, then prove the deploy.** Nothing has run end to end. `main` is still the old flat repo with no `package.json`, so Workers Builds fails at `npm ci` until this branch lands. Merge, confirm `name = "oboros"` matches the existing Worker in the Cloudflare dashboard, then push a trivially visible change and watch it reach oboros.app. A `name` mismatch fails by silently creating a second Worker rather than by erroring, so check it before rather than debug it after.

**2. Decide whether the suite should gate the deploy.** It does not today — Workers Builds ships without running it. A branch protection rule on `main` requiring the `check` run would put the gate at the merge. Worth doing before anyone else has push access; less urgent while it is one person who can read a red check.
