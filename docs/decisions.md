...
### Who deploys — reversed once, read both halves

**Originally chosen:** the GitHub Action runs `wrangler deploy`, gated on the full suite. **Rejected at the time:** letting Cloudflare's git integration deploy, on the grounds that it would mean committing a 1.3 MB generated `index.html` and taking a bot commit on every deploy.

**Now chosen:** Cloudflare Workers Builds deploys, and the Action is a check that does not deploy. The original objection turned out to be about a different product — Workers Builds runs `npm ci && npm run build` from source and uploads `dist/`, so nothing generated is committed and there is no bot commit. It was already connected and authenticated, where the Action needed two GitHub secrets that did not exist. One system deploys, because two would race on the same commit.

**What that costs, and it is not small:** the full suite no longer gates a deploy. Workers Builds runs no tests, so a red check does not stop a ship — it only tells you afterwards. The way to get the gate back without giving the deploy to the Action is a branch protection rule on `main` requiring `check` to pass, which makes the merge the gate rather than the deploy. That is not set up.

**Also rejected:** keeping both, with the Action deploying only when Workers Builds is disabled. Two deploy paths that are each correct only when the other is off is a configuration you have to remember, and the failure mode is silent.
- **The PWA icons are committed, and `npm run build` launches no browser.** `make-icons.js` used to run inside the build, which meant any machine that deployed had to download Chromium to redraw four PNGs from a vector file that changes approximately never. Now the icons live in `static/icons/`, ship via `copy-static.js`, and are regenerated by hand with `npm run icons`. Rejected: keeping generation in the build and installing Chromium in CI — it is slow on every deploy, it needs apt access that not every build container grants, and it buys nothing, because the rasterisation is deterministic and its input is one file. Accepted cost: change the mark and you must remember to run `npm run icons` and commit the result; nothing checks that for you.
